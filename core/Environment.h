...
public:
	...
	void SetUseMuscle(bool use_muscle){mUseMuscle = use_muscle;} // Inline getter/setter
	void SetControlHz(int con_hz) {mControlHz = con_hz;}
	void SetSimulationHz(int sim_hz) {mSimulationHz = sim_hz;}

	void SetCharacter(Character* character) {mCharacter = character;} // modern C++ style pointer, see if shared ptr is better
...
public:
	...
	const dart::simulation::WorldPtr& GetWorld(){return mWorld;} // Const reference to avoid copying
