...
class MultimodalPPO(BasePPO):
    ...
    def __init__(self, motion_list_path: str, metadata_template_path: str,
        ...
        # Use EpisodeBufferWithMotion for multimodal training
    ...
    def ComputeTDandGAE(self):
        ...
        Overrides base to track per-motion statistics.
        """
        ...
        for epi in self.total_episodes:
            ...
            motion_idx = motion_indices[0] // self.num_slaves_per_motion
            motion_name = self.motion_configs[motion_idx].name
    ...
    def GenerateTransitions(self):
        ...
        Overrides base to use EpisodeBufferWithMotion.
        """
        self.total_episodes = []
        ...
        while True:
            ...
            for j in range(self.num_slaves):
                ...
                if (np.any(np.isnan(states[j])) or np.any(np.isnan(actions[j])) or
                    ...
                    motion_idx = j  # Used to determine which motion this episode is from
                    self.episodes[j].Push(states[j], actions[j], reward, values[j], logprobs[j], motion_idx)
                    local_step += 1
